...
ggplot(threat_df) +
  ...
  geom_point(aes(alpha = ifelse(estimator == "Direct", 1, 0)),
             size = 3,
             color = nu_purple) +
  geom_linerange(aes(
    ymin = conf.low,
    ymax = conf.high,
    x = estimator,
    alpha = ifelse(estimator == "Direct", 1, 0)
  ),
  color = nu_purple,
  linewidth = 1) +
...
## Combining both refinements

```{r bounds-figure}
bnds = read.csv("list_placebo/bounds_kpp_direct.csv")

b = bnds %>% 
  filter(estimator == "DLE") %>% 
  mutate(
    ...
    list_treatment = fct_relevel(list_treatment,
                                 "Threaten", "Evict",
                                 "Donation", "Offer\nwork")
  ) %>% 
  mutate(p = case_when(
...
ggplot(b) +
  ...
  geom_linerange(
    aes(
      x = factor(p),
      ymin = estimate_comb_lo,
      ymax = estimate_comb_hi
    ),
    linewidth = 3, color = nu_purple
  ) +
  geom_linerange(
    aes(
      x = factor(p),
      ymin = boot_ci_comb_lo_lo,
      ymax = boot_ci_comb_hi_hi
    ),
    linewidth = 1, color = nu_purple
  ) + 
  labs(y = "Prevalence",
       x = "Proportion holding placebo") +
  ylim(0,1)
```
...
- **Paper:** Validation study with known proportion
